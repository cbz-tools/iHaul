...
use helpers::{
    W11_ACCENT, W11_BORDER, W11_HOVER, W11_PANEL, W11_SEL, W11_SEL_TEXT, W11_SURFACE, W11_TEXT,
    W11_TOOLBAR, apply_win11_theme, cmd_btn, danger_button, dialog_button, entry_kind, file_icon,
    flat_icon_btn, format_size, primary_button, read_clipboard_files, setup_font,
};
pub use types::{DeviceCommand, DeviceMessage, FileEntry};
use types::{
    DeviceStatus, FileLoadState, FileMetadata, FilePanelActions, SortColumn, TransferItem,
    TransferStatus,
};
...
use egui_material_icons::icons::{
    ICON_ARROW_BACK, ICON_ARROW_FORWARD, ICON_ARROW_UPWARD, ICON_CANCEL, ICON_CONTENT_PASTE,
    ICON_CREATE_NEW_FOLDER, ICON_DELETE, ICON_DOWNLOAD, ICON_DRIVE_FILE_RENAME_OUTLINE,
    ICON_FOLDER, ICON_INSERT_DRIVE_FILE, ICON_SEARCH, ICON_SETTINGS, ICON_UPLOAD,
};
use std::{
...
impl App {
    ...
    fn poll_messages(&mut self) {
        while let Ok(msg) = self.msg_rx.try_recv() {
            match msg {
                ...
                DeviceMessage::DownloadProgress {
                    filename,
                    bytes_done,
                    bytes_total,
                } => {
                ...
                DeviceMessage::DownloadDone(f) => {
                    self.mark_transfer_done(&f, false);
                }
    ...
    fn handle_transfer_started(&mut self, filename: &str, is_upload: bool) {
        if let Some(t) = self.transfers.iter_mut().rev().find(|t| {
            t.filename == filename
                && t.is_upload == is_upload
                && matches!(t.status, TransferStatus::Queued)
        }) {
    ...
    fn handle_transfer_progress(
        ...
        if let Some(t) = self.transfers.iter_mut().rev().find(|t| {
            t.filename == filename
                && t.is_upload == is_upload
                && matches!(t.status, TransferStatus::Active)
        }) {
            if is_upload && t.started_at.is_none() && bytes_done > 0 {
                t.started_at = Some(Instant::now());
            }
    ...
    fn has_active_transfers(&self) -> bool {
        self.transfers
            .iter()
            .any(|t| matches!(t.status, TransferStatus::Queued | TransferStatus::Active))
    }
    ...
    fn current_op_label<'a>(&self, s: &'a crate::i18n::S) -> Option<&'a str> {
        if self.is_renaming {
            return Some(s.op_renaming);
        }
        if self.is_deleting {
            return Some(s.op_deleting);
        }
        if self.is_creating_folder {
            return Some(s.op_creating_folder);
        }
        if self.is_preparing_export {
            return Some(s.op_preparing);
        }
        if matches!(self.file_load_state, FileLoadState::Loading) {
            return Some(s.op_loading);
        }
        None
    }
    ...
    fn trigger_export(&mut self, bundle_id: &str, dest: std::path::PathBuf) {
        ...
        self.cmd_tx
            .send(DeviceCommand::ExportFiles {
    ...
    fn navigate_up(&mut self) {
        if self.current_path == "/Documents" {
            return;
        }
    ...
    fn prune_filter_selection(&mut self, filter_lower: &str) {
        let visible_names: HashSet<String> = self
            ...
            .collect();
        self.selected_files
            .retain(|name| visible_names.contains(name));
        if self
            .anchor_file
            .as_ref()
            .is_some_and(|name| !visible_names.contains(name))
        {
            self.anchor_file = None;
        }
    }
...
impl App {
    ...
    fn handle_shortcuts(&mut self, ctx: &egui::Context, is_busy: bool) {
        ...
        let ctrl_v_win32 =
            !text_edit_focused && ctrl_v_held && !self.ctrl_v_was_held && !paste_event;
        ...
        let ctrl_shift_n =
            ctx.input(|i| i.key_pressed(egui::Key::N) && i.modifiers.ctrl && i.modifiers.shift);
        ...
        if !is_busy && !text_edit_focused {
            ...
            if ctrl_a && app_sel {
                self.selected_files = self.files.iter().map(|e| e.name.clone()).collect();
            }
            if del_key && has_sel {
                self.trigger_delete_confirm();
            }
            let sel_has_dir = self
                .selected_files
                .iter()
    ...
    fn show_sidebar(&mut self, ui: &mut egui::Ui) {
        ...
        if self.apps.is_empty() {
            ...
            ui.horizontal(|ui| {
                ...
                ui.colored_label(
                    ...
                    egui::RichText::new(hint).size(12.0),
                );
            });
            return;
        }
        ...
        for &real_idx in &all_sorted {
            ...
            if !self.app_icons.contains_key(&bundle_id) {
                ...
                let decoded = self.apps[real_idx]
                    .icon_png
                    .as_deref()
    ...
    fn show_transfer_panel(&mut self, ui: &mut egui::Ui) {
        ...
        let done_count = self
            .transfers
            .iter()
            .filter(|t| matches!(t.status, TransferStatus::Done))
            .count();
        ...
        if failed_count > 0 {
            info += &format!(" · {}{}", failed_count, s.failed_suffix);
        }
        ...
        let speed_bps: f64 = if self.speed_checkpoints.len() >= 2 {
            ...
            if dt > 0.0 {
                (b_new - b_old) as f64 / dt
            } else {
                0.0
            }
        } else {
            let batch_started = self.transfers.iter().filter_map(|t| t.started_at).min();
            batch_started
                .map(|t0| {
        ...
        ui.horizontal(|ui| {
            ...
            ui.add_enabled_ui(can_act, |ui| {
                if cmd_btn(ui, ICON_CREATE_NEW_FOLDER, s.lbl_new_folder)
                    .on_hover_text(s.tip_new_folder)
                    .clicked()
                {
                    self.trigger_new_folder();
                }
            ...
            ui.separator();
            ui.add_enabled_ui(
                can_act && has_sel && self.selected_files.len() == 1 && !sel_has_dir,
                |ui| {
                    if cmd_btn(ui, ICON_DRIVE_FILE_RENAME_OUTLINE, s.lbl_rename)
                        .on_hover_text(s.tip_rename)
                        .clicked()
                    {
                        self.trigger_rename();
                    }
        ...
        ui.horizontal(|ui| {
            let status_label: Option<String> = if self.has_active_transfers() {
                let active_count = self
                    ...
                    .count();
                let parallel_str = if active_count > 1 {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                ui.allocate_ui_with_layout(
                    ...
                    |ui| {
                        ...
                        ui.label(egui::RichText::new(ICON_SEARCH.codepoint.to_string()).font(
                            egui::FontId::new(
                                FILTER_ICON_SIZE,
                ...
                if !self.filter_text.is_empty() {
                    ...
                    ui.painter().text(
                        ...
                        egui::FontId::new(14.0, egui::FontFamily::Name("material-icons".into())),
                        W11_TEXT,
                    );
                ...
                macro_rules! hdr_col {
                    ($label:expr, $col:expr) => {
                        header.col(|ui| {
                            let r = ui.add(
                                egui::Label::new(
                                    egui::RichText::new(format!("{}{}", $label, arrow($col)))
                                        .strong(),
                                )
                ...
                for (i, entry) in display_files.iter().enumerate() {
                    ...
                    let mk = || {
                        ...
                        move |ui: &mut egui::Ui| {
                            ...
                            if can_act {
                                ...
                                ui.separator();
                                if ui
                                    .add_enabled(has_sel, egui::Button::new(s.ctx_export))
                                    .clicked()
                                {
                                    ed.set(true);
                                    ui.close();
                                ...
                                if ui
                                    .add_enabled(
                                        selected_files.len() == 1 && !sel_has_dir,
                                        egui::Button::new(s.ctx_rename),
                                    )
                                    .clicked()
                                {
                                    rd.set(true);
                                ...
                                ui.separator();
                                if ui
                                    .add_enabled(has_sel, egui::Button::new(s.ctx_delete))
                                    .clicked()
                                {
                                    dd.set(true);
                                    ui.close();
                    ...
                    body.row(26.0, |mut row| {
                        ...
                        row.col(|ui| {
                            ...
                            let r = ui.interact(
                                full_rect,
                                egui::Id::new(("row_col1", i)),
                                egui::Sense::click(),
                            );
                            if r.hovered() {
                                new_hovered.set(Some(i));
                            }
                            if show_hover {
                                let row_rect =
                                    egui::Rect::from_x_y_ranges(table_x_range, full_rect.y_range());
                                ui.painter().with_clip_rect(row_rect).set(
                                    bg_slot,
                                    egui::Shape::rect_filled(row_rect, 0.0, W11_HOVER),
                                );
                            }
                            if r.secondary_clicked() && !is_sel {
                                row_right.set(true);
                            }
                            if r.double_clicked() && entry.is_dir {
                                row_dbl = true;
                            } else if r.clicked() {
                                row_sel = Some((
                        ...
                        row.col(|ui| {
                            let full_rect = ui.max_rect();
                            if let Some((_, m)) = file_info.get(&entry.name) {
                                ui.label(m.as_str());
                            }
                            let r = ui.interact(
                                full_rect,
                                egui::Id::new(("row_col4", i)),
                                egui::Sense::click(),
                            );
                            if r.hovered() {
                                new_hovered.set(Some(i));
            ...
            if bg_rect.height() > 0.0 {
                ...
                bg_resp.context_menu(|ui| {
                    ...
                    if can_act {
                        ...
                        if ui
                            .add_enabled(
                                has_sel && selected_files.len() == 1 && !sel_has_dir,
                                egui::Button::new(s.ctx_rename),
                            )
                            .clicked()
                        {
    ...
    fn apply_selection(&mut self, sel: (usize, String, bool, bool), display_names: &[String]) {
    ...
    fn show_file_panel(&mut self, ui: &mut egui::Ui, ctx: &egui::Context, is_busy: bool) {
        ...
        let mut display_files: Vec<&FileEntry> = self
            .files
            .iter()
            .filter(|entry| {
                filter_lower.is_empty() || entry.name.to_lowercase().contains(&filter_lower)
        ...
        if !is_busy && !display_names.is_empty() {
            ...
            if arrow_up || arrow_down {
                ...
                if shift {
                    ...
                    self.selected_files.clear();
                    self.selected_files
                        .insert(display_names[new_cursor].clone());
        ...
        if let Some(col) = actions.sort_click {
    ...
    fn show_settings_dialog(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new(s.settings_title)
            ...
            .show(ctx, |ui| {
                        ...
                        egui::ComboBox::new("lang_combo", "")
                            ...
                            .show_ui(ui, |ui| {
                                for lang in [
                                    ...
                                    if ui
                                        .selectable_label(self.settings.lang == lang, lang.label())
                                        .clicked()
                                    {
                ...
                ui.horizontal(|ui| {
                    if ui.button(s.settings_reset_defaults).clicked() {
                        do_reset = true;
                    }
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if primary_button(ui, s.btn_ok).clicked() {
                            do_ok = true;
                        }
                        if dialog_button(ui, s.btn_cancel).clicked() {
                            do_cancel = true;
                        }
                    });
                });
    ...
    fn show_delete_dialog(&mut self, ctx: &egui::Context, s: &crate::i18n::S) {
        ...
        egui::Window::new(s.del_confirm_title)
            ...
            .show(ctx, |ui| {
                ...
                if n == 1 {
                    ...
                    let msg = format!(
                        "{}{}{}",
                        s.del_confirm_pre_multi, n, s.del_confirm_suf_multi
                    );
                ...
                ui.horizontal(|ui| {
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if danger_button(ui, s.btn_delete).clicked() || enter {
                            do_delete = true;
                        }
                        if dialog_button(ui, s.btn_cancel).clicked() {
                            cancel_del = true;
        ...
        if do_delete {
            if let Some(bundle_id) = bid {
                ...
                self.is_deleting = true;
                self.cmd_tx
                    .send(DeviceCommand::DeleteFiles {
                        bundle_id,
                        current_path: cur,
                        abs_paths,
                    })
        ...
        egui::Window::new(s.rename_title)
            ...
            .show(ctx, |ui| {
                egui::Grid::new("rename_grid")
                    ...
                    .show(ui, |ui| {
                        ui.label(
                            egui::RichText::new(s.rename_current_prefix)
                                .color(egui::Color32::from_gray(130))
                                .size(12.0),
                        );
                        if let Some(old) = &self.renaming_file {
                            ui.label(
                                egui::RichText::new(old.as_str())
                ...
                if self.rename_just_opened {
                    ...
                    let mut state = egui::text_edit::TextEditState::load(ctx, rename_edit_id)
                        .unwrap_or_default();
                    state
                        .cursor
                ...
                ui.horizontal(|ui| {
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        let apply = if valid {
                            ...
                            ui.add_enabled(
                                false,
                                egui::Button::new(s.btn_change)
                                    .min_size(egui::Vec2::new(80.0, 28.0)),
                            )
                        };
                        if apply.clicked() || (valid && enter) {
                            submit = true;
                        }
                        if dialog_button(ui, s.btn_cancel).clicked() {
                            cancel_rename = true;
    ...
    fn show_new_folder_dialog(&mut self, ctx: &egui::Context, s: &crate::i18n::S) {
        ...
        egui::Window::new(s.new_folder_title)
            ...
            .show(ctx, |ui| {
                ui.label(
                    egui::RichText::new(s.new_folder_hint)
                        .color(egui::Color32::from_gray(100))
                        .size(12.0),
                );
                ui.add_space(6.0);
                ...
                if self.new_folder_just_opened {
                    resp.request_focus();
                    self.new_folder_just_opened = false;
                }
                if resp.lost_focus()
                    && !ctx.input(|i| i.pointer.any_pressed())
                    && !ctx.input(|i| i.key_pressed(egui::Key::Escape))
                ...
                ui.horizontal(|ui| {
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        let create = if valid {
                            ...
                            ui.add_enabled(
                                false,
                                egui::Button::new(s.btn_create)
                                    .min_size(egui::Vec2::new(80.0, 28.0)),
                            )
                        };
                        if create.clicked() || (valid && enter) {
                            submit = true;
                        }
                        if dialog_button(ui, s.btn_cancel).clicked() {
                            cancel_new = true;
...
fn sidebar_row(
    ...
    if let Some(tex) = texture {
        let icon_rect =
            egui::Rect::from_center_size(egui::pos2(icon_x + 12.0, cy), egui::Vec2::splat(24.0));
    ...
    let name_rect =
        egui::Rect::from_min_max(egui::pos2(name_x, content_rect.min.y), content_rect.max);
...
fn sidebar_section_header(ui: &mut egui::Ui, label: &str) {
    ...
    ui.painter()
        .rect_filled(rect, 0.0, egui::Color32::from_gray(238));
    ui.painter().text(
...
impl eframe::App for App {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            egui::Panel::bottom("transfer_panel").show_inside(ui, |ui| {
