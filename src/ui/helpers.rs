...
use egui_material_icons::icons::{
    ICON_AUDIO_FILE, ICON_FOLDER, ICON_FOLDER_ZIP, ICON_IMAGE, ICON_INSERT_DRIVE_FILE,
    ICON_PICTURE_AS_PDF, ICON_VIDEOCAM,
};
...
pub(super) fn entry_kind(entry: &FileEntry, folder_label: &str) -> String {
    if entry.is_dir {
        return folder_label.to_string();
    }
    entry
        .name
        .rfind('.')
        .map(|i| entry.name[i + 1..].to_uppercase())
...
fn append_font(fonts: &mut egui::FontDefinitions, font_name: &str, data: Vec<u8>, index: u32) {
